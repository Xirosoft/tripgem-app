  ...
  (newPermissions) => {
    router.options.routes.forEach((route) => {
      if (route.meta && route.meta.permissionName) {
        ...
        if (permission) {
          route.meta.permissions = `${permission.can_read ? 'read' : ''} | ${permission.can_write ? 'write' : ''} | ${permission.can_create ? 'create' : ''}`
            .replace(/\s\|\s\|/g, '')
            .trim()
        }
      }
    })
    // Force a route refresh to re-evaluate permissions
    if (router.currentRoute.value.name) {
      router.replace({ name: 'refresh', params: { nextUrl: router.currentRoute.value.fullPath } })
    }
  },
  { deep: true },
)
