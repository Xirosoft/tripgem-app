  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        // Create a clean data object without the logo_url
        const cleanData = { ...this.formData }
        delete cleanData.logo_url // Remove logo_url from data

        // Convert social_media_links object to string if it's an object
        if (typeof cleanData.social_media_links === 'object') {
          cleanData.social_media_links = JSON.stringify(cleanData.social_media_links)
        }

        // Append the main data as a JSON string
        formData.append('data', cleanData)
