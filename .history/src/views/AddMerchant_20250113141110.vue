  ...
  methods: {
    ...
    async loadCountries() {
      try {
        const options = {
          method: 'GET',
          headers: {
            'X-RapidAPI-Key': 'fd09aa16a9msh16fe0226ca85e00p131604jsna2c1f9f7fa68', // Get free key from RapidAPI
            'X-RapidAPI-Host': 'https://wft-geo-db.p.rapidapi.com/v1/geo/countries/US/places',
          },
        }

        const response = await fetch(
          'https://wft-geo-db.p.rapidapi.com/v1/geo/countries?limit=200',
          options,
        )
        const result = await response.json()

        this.locationData.countries = result.data
          .map((country) => ({
            id: country.code,
            name: country.name,
    ...
    async loadProvinces() {
      ...
      try {
        const options = {
          method: 'GET',
          headers: {
            'X-RapidAPI-Key': 'fd09aa16a9msh16fe0226ca85e00p131604jsna2c1f9f7fa68',
            'X-RapidAPI-Host': 'wft-geo-db.p.rapidapi.com',
          },
        }

        const response = await fetch(
          `https://wft-geo-db.p.rapidapi.com/v1/geo/countries/${this.formData.country}/regions?limit=100`,
          options,
        )
        const result = await response.json()

        this.locationData.provinces = result.data.map((region) => ({
    ...
    async loadCities() {
      ...
      try {
        ...
        const response = await fetch(
          `https://wft-geo-db.p.rapidapi.com/v1/geo/cities?countryIds=${this.formData.country}&regionId=${this.formData.province}&limit=100`,
          options,
        )
        const result = await response.json()

        this.locationData.cities = result.data.map((city) => ({
          id: city.id.toString(),
          name: city.name,
          zip: city.postalCode || '',
        }))
    ...
    async updateZipCode() {
      ...
      try {
        // Find the selected city in the current cities array
        const selectedCity = this.locationData.cities.find((city) => city.id === this.formData.city)
        if (selectedCity) {
          this.formData.zip_code = selectedCity.zip
        } else {
          this.formData.zip_code = ''
        }
      } catch (error) {
        ...
        $('#country-select')
          .select2({
            ...
            templateResult: (country) => {
              ...
              const $country = $(
                `<span><img src="${country.element.dataset.flag}" class="flag-icon"/> ${country.text}</span>`,
              )
              return $country
            },
          })
