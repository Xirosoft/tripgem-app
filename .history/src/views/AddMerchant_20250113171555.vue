...
export default {
  ...
  methods: {
    ...
    async loadProvinces() {
      ...
      // Only load provinces if Philippines is selected
      if (this.formData.country === 'PH') {
        try {
          const response = await axios.get(`${this.PH_API_URL}/provinces`)

          if (response.data && response.data.data) {
            this.locationData.provinces = response.data.data
              .map((province) => ({
                id: province.id,
                name: province.name,
              }))
        ...
        // For other countries, use the OpenCage API as before
        try {
          const response = await axios.get(
            `https://api.opencagedata.com/geocode/v1/json?q=${this.formData.country}&key=${this.OPENCAGE_API_KEY}&limit=1`,
          )
          // ...existing OpenCage handling code...
        } catch (error) {
          console.error('Failed to load provinces:', error)
          this.toast.error('Failed to load provinces')
        }
    ...
    async loadCities() {
      ...
      if (this.formData.country === 'PH') {
        try {
          const response = await axios.get(
            `${this.PH_API_URL}/cities-by-province?province_id=${this.formData.province}`,
          )
        ...
        // Use OpenCage API for other countries
        // ...existing OpenCage handling code...
      }
    },
    ...
    async updateZipCode() {
      ...
      if (this.formData.country === 'PH') {
        try {
          // For Philippines, we can try to get the ZIP code from the barangays API
          const response = await axios.get(
            `${this.PH_API_URL}/barangays-by-city?city_id=${this.formData.city}`,
          )

          if (response.data && response.data.data && response.data.data.length > 0) {
            // Some barangays might have ZIP codes
            const firstBarangay = response.data.data[0]
            if (firstBarangay.zip_code) {
              this.formData.zip_code = firstBarangay.zip_code
            }
          }
        } catch (error) {
          console.error('Failed to get ZIP code:', error)
          // Don't show error toast as ZIP code is optional
        }
      } else {
        // Use OpenCage API for other countries
        // ...existing OpenCage handling code...
