...
export default {
  ...
  data() {
    return {
      ...
      formData: {
        ...
        logo_url: { url: '' }, // Ensure logo_url is initialized
        cover_photo: { url: '' }, // Ensure cover_photo is initialized
      },
      businessTypes: [
  ...
  methods: {
    initializeComponents() {
      ...
      // Initialize Dropzone for cover photo
      const dropzoneCoverPhoto = document.querySelector('#dropzone-cover-photo')
      if (dropzoneCoverPhoto) {
        this.dropzoneCoverPhoto = new Dropzone(dropzoneCoverPhoto, {
          url: `${config.apiUrl}/upload`,
          headers: config.getHeaders(),
          previewTemplate,
          parallelUploads: 1,
        ...
        this.dropzoneCoverPhoto.on('addedfile', (file) => {
          if (this.dropzoneCoverPhoto.files.length > 1) {
            this.dropzoneCoverPhoto.removeFile(this.dropzoneCoverPhoto.files[0])
          }
          this.handleCoverPhotoUpload(file)
        })

        this.dropzoneCoverPhoto.on('removedfile', () => {
          this.uploadedCoverPhoto = null
          this.formData.cover_photo.url = ''
        })

        this.dropzoneCoverPhoto.on('error', (file) => {
          this.dropzoneCoverPhoto.removeFile(file)
        })

        this.dropzoneCoverPhoto.on('success', (file, response) => {
          console.log('Upload success:', response)
        })
      }
    ...
    handleCoverPhotoUpload(file) {
      ...
      this.formData.cover_photo.url = URL.createObjectURL(file) // Ensure URL is set
    },
    ...
    async submitForm() {
      try {
        ...
        console.log('Logo photo:', this.formData.logo_url)
        console.log('Cover photo:', this.formData.cover_photo)
    ...
    cleanupComponents() {
      try {
        ...
        if (this.dropzoneCoverPhoto) this.dropzoneCoverPhoto.destroy()
            ...
            <div class="col-12 mb-4">
              ...
              <div class="dropzone" id="dropzone-cover-photo">
                <div class="dz-message needsclick">
                  ...
                  <h5 class="mb-0">Drop files here or click to upload</h5>
                  <span class="text-muted">Allowed JPG, JPEG, PNG, GIF. Max size of 5MB.</span>
                  <div v-if="formData.cover_photo.url" class="mt-3">
                    <img :src="formData.cover_photo.url" alt="Preview" style="max-width: 200px" />
                    <p class="text-success mt-2">Cover photo uploaded successfully</p>
                  </div>
                </div>
              </div>
              <small class="text-danger" v-if="!formData.cover_photo.url"
