...
export default {
  ...
  data() {
    return {
      ...
      OPENCAGE_API_KEY: 'YOUR_OPENCAGE_API_KEY_HERE', // Replace with your actual API key
    }
  },
  async created() {
    try {
    ...
    async loadCountries() {
      try {
        const response = await axios.get(
          `https://api.opencagedata.com/geocode/v1/json?q=countries&key=${this.1e66ba3ae6cd48629e50f28f7107b184}&limit=1`,
        )

        // OpenCage doesn't provide a direct countries list, so we'll use a predefined list
        // This is a simplified example - you might want to use a more complete country list
        this.locationData.countries = [
    ...
    async loadProvinces() {
      ...
      try {
        ...
        if (response.data.results && response.data.results[0]) {
          ...
          // Extract state/province information from components
          // Note: This is a simplified example - you might need to adjust based on the country
          this.locationData.provinces = Object.keys(components)
            .filter((key) => key.includes('state') || key.includes('province'))
            .map((key) => ({
              id: components[key],
              name: components[key],
            }))
