...
export default {
  ...
  data() {
    return {
      ...
      error: null,
      loading: false,
      discounts: [], // Add discounts property
    }
  },
  methods: {
    ...
    async fetchDiscounts() {
      try {
        const response = await axios.get(`${config.apiUrl}/discounts`, {
          headers: config.getHeaders(),
        })
  ...
  mounted() {
    ...
    this.fetchDiscounts() // Fetch discounts on mount
    this.$nextTick(() => {
      $('#multicol-country')
        .select2({
          allowClear: true,
          ...
          <div class="col-md-6 mb-6">
            ...
            <div class="input-group input-group-merge">
              ...
              <select class="form-select" v-model="booking.discount" id="discount" required>
                ...
                <option v-for="discount in discounts" :key="discount.name">
