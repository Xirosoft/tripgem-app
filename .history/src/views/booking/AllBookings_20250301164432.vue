...
export default {
  ...
  mounted() {
    this.$nextTick(() => {
      ...
      this.$nextTick(() => {
        if (this.table) {
          console.log('Table initialized:', this.table) // Debugging log
          this.$refs.filterComponent.table = this.table // Set the table prop for the FilterComponent
          this.$refs.filterComponent.addFilters() // Ensure filters are added after table initialization
        } else {
          console.error('Table initialization failed') // Log error if table is not initialized
        }
      })
    })
  },
  ...
  methods: {
    ...
    initializeDataTable() {
      ...
      if (!this.filtersAdded) {
        this.$refs.filterComponent.addFilters()
...
<template>
  <div>
    ...
    <table ref="bookingsTable" class="table table-striped">
      <thead>
        <tr>
          <th>Booking ID</th>
          <th>Full Name</th>
