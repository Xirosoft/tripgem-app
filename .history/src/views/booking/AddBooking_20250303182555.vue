  ...
  data() {
    return {
      booking: {
        ...
        phone: '',
        travel_date: '',
        discount: '', // Change discount to a string to hold the selected discount
        transport_types: '',
        adult_price: '',
        child_price: '',
        total_price: '',
  ...
  methods: {
    async fetchTourData(tourId) {
      ...
      try {
        ...
        this.booking.discount = JSON.stringify(this.tour.discount) // Ensure discount is a JSON string
      } catch (error) {
    ...
    parseDiscount(discount) {
      ...
      try {
        const discountJson = JSON.parse(discount)
        return discountJson.map((d) => ({
          name: d.name,
          amount: d.amount,
          target: d.target,
        }))
      } catch (error) {
        console.error('Error parsing discount as JSON:', error)
        // Handle non-JSON strings gracefully
        return discount.split(',').map((d) => {
          const parts = d.split('-')
          return {
            name: parts[0].trim(),
            amount: parts[1] ? parseFloat(parts[1].trim()) : 0,
            target: parts[2] ? parts[2].trim() : '',
          }
        })
      }
    },
        ...
        <div class="row">
          ...
          <div class="col-md-6 mb-6">
            ...
            <div class="input-group input-group-merge">
              ...
              <select class="form-select" v-model="booking.discount" id="discount" required>
                <option value="">Select Discount</option>
                <option
                  v-for="discount in parseDiscount(booking.discount)"
                  :key="discount.name"
                  :value="discount.name"
                >
                  {{ discount.name }} - {{ discount.amount }} ฿ off {{ discount.target }}
                </option>
              </select>
            </div>
          </div>
