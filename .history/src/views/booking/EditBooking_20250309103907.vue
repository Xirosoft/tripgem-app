...
export default {
  ...
  methods: {
    ...
    parseDropLocations(drop_time) {
      if (!drop_time || typeof drop_time !== 'string') {
        return []
      }
      try {
        const sanitizedInput = drop_time.trim() // Remove extra spaces/newlines
        if (sanitizedInput.startsWith('[') && sanitizedInput.endsWith(']')) {
          const parsedLocation = JSON.parse(sanitizedInput)
          if (!Array.isArray(parsedLocation)) {
            console.error('Expected an array but got:', parsedLocation)
            return []
          }
          return parsedLocation.map((loc) => ({
            location: loc.location,
          ...
          console.error('Input is not a valid JSON array:', sanitizedInput)
          return []
        }
      } catch (error) {
        console.error('Error parsing drop locations:', error, 'Input:', drop_time)
        return []
      }
    },
