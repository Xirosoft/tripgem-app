...
export default {
  ...
  methods: {
    initializeDataTable() {
      ...
      this.table = $(this.$refs.permissionsTable).DataTable({
        ...
        columns: [
          ...
          { data: 'created_at' }, // Created date field
          {
            ...
            render: function () {
              return `
                <button class="btn btn-sm btn-primary edit-permission"><i class="ti ti-edit ti-md"></i>Edit</button>
                <button class="btn btn-sm btn-danger delete-permission"><i class="ti ti-trash ti-md"></i> Delete</button>
              `
            },
          },
        ],
      ...
      // Bind delete event
      $(this.$refs.permissionsTable).on('click', '.delete-permission', (e) => {
        const id = $(e.currentTarget).closest('tr').find('td').eq(1).text()
    ...
    async deletePermission(id) {
      if (confirm('Are you sure you want to delete this permission?')) {
        await this.permissionStore.deletePermission(id)
      }
    },
  },
  watch: {
...
<template>
  ...
  <div class="card">
    ...
    <div class="card-datatable table-responsive">
      ...
      <div v-if="isLoading" class="text-center my-4 tripgem_loader">
...
.text-center.my-4.tripgem_loader {
  position: absolute;
  width: 100%;
  height: 100%;
  background: #fff;
