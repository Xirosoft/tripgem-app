...
export default {
  ...
  methods: {
    ...
    async loadCountries() {
      try {
        // Get countries list
        const countriesResponse = await axios.get(this.API_URLs.countries)
        ...
        if (countriesResponse.data && countriesResponse.data.data) {
          ...
          for (const country of countries) {
            try {
              const flagResponse = await axios.post(this.API_URLs.flags, {
                iso2: country.id,
              })
              if (flagResponse.data && flagResponse.data.data && flagResponse.data.data.flag) {
                country.flag = flagResponse.data.data.flag
              }
            } catch (flagError) {
              console.warn(`Failed to load flag for ${country.name}:`, flagError)
            }
          }

          this.locationData.countries = countries
        }
      } catch (error) {
