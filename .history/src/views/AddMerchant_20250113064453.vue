...
export default {
  ...
  methods: {
    ...
    handleFileUpload(event, type) {
      ...
      // Update formData with filenames
    ...
    async uploadFile(file) {
      ...
      try {
        const response = await fetch(`${config.apiUrl}/upload/add`, {
          method: 'POST',
          body: formData,
          headers: {
            // Let browser set multipart/form-data header with boundary
    ...
    async submitForm() {
      ...
      try {
        ...
        if (this.uploadedLogo instanceof File) {
          uploadedUrls.logo = await this.uploadFile(this.uploadedLogo)
        }

        // Upload all document files
        for (const [key, files] of Object.entries(this.uploadedFiles)) {
          if (files && files.length) {
            uploadedUrls[key] = await Promise.all(files.map((file) => this.uploadFile(file)))
          }
        }
        ...
        const submitData = {
          ...
          business_permits: uploadedUrls.business_permits,
          membership_certificates: uploadedUrls.membership_certificates,
