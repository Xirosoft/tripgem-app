...
const loadTourDetails = async () => {
  try {
    ...
    setTimeout(() => {
      $('#language').val(formData.value.languages_supported?.map(lang => lang?.toUpperCase())).trigger('change')
      $('#currency').val(formData.value.currency?.map(curr => curr?.toUpperCase())).trigger('change')
      ...
      $('#status-org').val(formData.value.status?.charAt(0).toUpperCase() + formData.value.status?.slice(1)).trigger('change')
    }, 0)
    }, 0)
  } catch (error) {
