...
import { DragAndDropUpload } from '../../utils/handleFileUpload'
...
const handleFilePondInit = (pond, fieldName) => {
  pond.on('processfile', async (error, file) => {
    if (!error) {
      const url = await DragAndDropUpload(file.file, formData.value, pond, fieldName)
      if (fieldName === 'image_gallery') {
        formData.value.image_gallery.push(url)
      } else if (fieldName === 'video_gallery') {
                ...
                <div class="tab-content p-0 ps-md-4">
                  ...
                  <div class="tab-pane fade show active" id="photoGallery" role="tabpanel">
                    ...
                    <FilePond
                      ...
                      allow-multiple
                      @init="(pond) => handleFilePondInit(pond, 'image_gallery')"
                    />
                  </div>
                  <!-- videoGallery Tab -->
                  <div class="tab-pane fade" id="videoGallery" role="tabpanel">
                    ...
                    <FilePond
                      ...
                      allow-multiple
                      @init="(pond) => handleFilePondInit(pond, 'video_gallery')"
                    />
                  </div>
                </div>
              </div>
        ...
        <div class="card mb-6">
          ...
          <div class="card-body">
            <FilePond
              name="thumbnail"
              accepted-file-types="image/*"
              @init="(pond) => handleFilePondInit(pond, 'thumbnail')"
            />
          </div>
        </div>
      </div>
