...
onMounted(async () => {
  ...
  const imageGalleryElement = document.querySelector('#image_gallery .dz-message')
  formData.value.image_gallery.forEach((image, index) => {
    if (!document.querySelector(`#image_gallery .dz-preview[data-index="${index}"]`)) {
      ...
      `
      imageGalleryElement.insertAdjacentElement('beforebegin', imagePreview)
    }
  })

  // Display existing videos in the gallery
  const videoGalleryElement = document.querySelector('#video_gallery .dz-message')
  formData.value.video_gallery.forEach((video, index) => {
    if (!document.querySelector(`#video_gallery .dz-preview[data-index="${index}"]`)) {
      const videoPreview = document.createElement('div')
      videoPreview.classList.add('dz-preview', 'dz-processing', 'dz-video-preview', 'dz-complete')
      videoPreview.setAttribute('data-index', index)
      ...
      `
      videoGalleryElement.insertAdjacentElement('beforebegin', videoPreview)
    }
  })
      ...
      <div class="col-12 col-lg-4">
              ...
              <div class="w-25 d-flex justify-content-end">
                <div class="form-check form-switch me-n3">
                  <input
                    type="checkbox"
                    class="form-check-input"
                    :checked="Boolean(formData.booking_enabled) == 1"
        ...
        <div class="card mb-6">
          ...
          <div class="card-body">
            ...
            />
            <TourTags :selectedTags="FilterTags" @tags-change="handleTagsChange" />
          </div>
        </div>
        <!-- /Organize Card -->
