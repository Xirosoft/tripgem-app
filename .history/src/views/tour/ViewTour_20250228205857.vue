...
export default {
  setup() {
    ...
    onMounted(async () => {
      ...
      // Ensure discount is an array
      if (typeof tourDetails.value.discount === 'string') {
        tourDetails.value.discount = JSON.parse(tourDetails.value.discount)
      }
    })
...
<template>
  <!-- Content -->
  <div class="row g-6" v-if="tourDetails">
    <div class="col-lg-8">
      <div class="card">
        <div class="card-body">
          ...
          <div class="card-body pt-4">
            ...
            <p><strong>Net Price (Adult):</strong> {{ tourDetails.net_price_adult }}</p>
            <p><strong>Net Price (Child):</strong> {{ tourDetails.net_price_child }}</p>
            <p><strong>Local Net Price (Adult):</strong> {{ tourDetails.local_net_price_adult }}</p>
            <p><strong>Local Net Price (Child):</strong> {{ tourDetails.local_net_price_adult }}</p>
            ...
            <p>
              ...
              <span v-for="cat in tourDetails.category" :key="cat.id">{{ cat.category_name }}</span>
            </p>
          ...
          >
            <div class="accordion-body py-4">
              ...
              <ul>
                ...
                <li>Local Adult Park Fee: {{ tourDetails.park_fee.local_price_adult_park_fee }}</li>
                <li>Local Child Park Fee: {{ tourDetails.park_fee.local_price_child_park_fee }}</li>
              </ul>
              ...
              <p>
                ...
                <span v-for="discount in tourDetails.discount" :key="discount.name">
                  {{ discount.name }} - {{ discount.amount }} ({{ discount.target }})
                </span>
              </p>
              <p>
