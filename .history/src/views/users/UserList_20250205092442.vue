...
export default {
  ...
  methods: {
    initializeDataTable() {
            ...
            defaultContent: `
              <div class="d-inline-block">
                ...
                <div class="dropdown-menu">
                  ${this.uCan('write', 'manage_users') ? '<a class="dropdown-item edit-user" href="javascript:void(0);"><i class="ti ti-pencil me-1"></i>Edit</a>' : ''}
                  ${this.uCan('read', 'manage_users') ? '<a class="dropdown-item view-user" href="javascript:void(0);"><i class="ti ti-eye me-1"></i>View</a>' : ''}
                  ${this.uCan('write', 'manage_users') ? '<a class="dropdown-item text-danger delete-user" href="javascript:void(0);"><i class="ti ti-trash me-1"></i>Delete</a>' : ''}
                </div>
              </div>
            `,
          },
      ...
      $(this.$refs.usersTable).on('click', '.delete-user', (e) => {
        const row = $(e.currentTarget).closest('tr')
        const userId = this.table.row(row).data().user_id
      ...
      // Add view user handler
      $(this.$refs.usersTable).on('click', '.view-user', (e) => {
        const row = $(e.currentTarget).closest('tr')
        const userId = this.table.row(row).data().user_id
        this.$router.push({
          name: 'ViewUser',
