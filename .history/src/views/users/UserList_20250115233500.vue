...
export default {
  ...
  async created() {
    ...
    this.userRoleStore = useUsersStore()
    await this.usersListStore.fetchUsers()
    await this.fetchRolesForUsers() // Fetch roles for users
    this.initializeDataTable() // Initialize DataTable after fetching roles
  },
  beforeUnmount() {
    if (this.table) {
      this.table.destroy()
  ...
  methods: {
    async fetchRolesForUsers() {
      const users = this.usersListStore.getAllUsers
      console.log(users)

      for (const user of users) {
        user.role_name = await this.userRoleStore.fetchUserRole(user.role_id) // Fetch and assign role_name
      }
    },
    initializeDataTable() {
      if (this.table) {
      ...
      this.table = $(this.$refs.usersTable).DataTable({
        console.log(data => data), // Add this line to see the data being passed to DataTable  ;
        ...
        columns: [
          ...
          { data: 'role_id' }, // Updated to display role_name
          { data: 'email' },
          { data: 'phone_number' },
          {
            data: 'status',
