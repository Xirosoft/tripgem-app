    ...
    async uploadFile(file, type = 'general') {
      ...
      try {
        ...
        if (typeof result === 'string') {
          ...
          // Nested data.url structure
          fileUrl = result.data.url
        } else if (result.data && result.data.file_url) {
          // Nested data.file_url structure
          fileUrl = result.data.file_url
        } else if (result.url) {
          // Direct url property
          fileUrl = result.url
        } else if (result.file_url) {
          // Direct file_url property
          fileUrl = result.file_url
        } else if (result.data && typeof result.data === 'string') {
          // Nested string URL in data
          fileUrl = result.data
        }
        ...
        if (!fileUrl || typeof fileUrl !== 'string') {
          ...
          throw new Error('Invalid file URL format in server response')
        }
