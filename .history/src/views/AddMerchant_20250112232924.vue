  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        ...
        // Append other files with their original names
        Object.entries(this.uploadedFiles).forEach(([key, files]) => {
          if (files && files.length) {
            files.forEach((file, index) => {
              if (file instanceof File) {
                // Store the filename in the corresponding array in submitData
                submitData[key][index] = file.name
        ...
        // Update the data in formData with the modified submitData
        formData.set('data', JSON.stringify(submitData))
