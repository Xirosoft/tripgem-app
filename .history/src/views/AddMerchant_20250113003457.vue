...
export default {
  ...
  methods: {
    ...
    handleFileUpload(event, type) {
      ...
      this.uploadedFiles[type].files = files

      // Create preview URLs for images
      this.uploadedFiles[type].urls = files.map((file) => {
        if (file.type.startsWith('image/')) {
          return URL.createObjectURL(file)
        }
        // For PDFs, you might want to use a PDF icon or thumbnail
        return file.name
      })

      // Update formData with the file names
      this.formData[type] = files.map((file) => file.name)
    },
    ...
    async uploadFile(file) {
      ...
      try {
        ...
        return data.url // Assuming server returns the stored file URL
      } catch (error) {
    ...
    async submitForm() {
      ...
      try {
        // Create a clean data object that matches the required format
        const submitData = {
          ...this.formData,
          ...
          // Convert established_year to number
          established_year: Number(this.formData.established_year),
        ...
        Object.entries(submitData).forEach(([key, value]) => {
          if (key === 'social_media_links') {
            // Append each social media link separately
            Object.entries(value).forEach(([socialKey, socialValue]) => {
              formData.append(`social_media_links[${socialKey}]`, socialValue)
            })
          } else if (Array.isArray(value)) {
            // Handle arrays (like branch_locations)
        ...
        if (this.uploadedLogo instanceof File) {
          const logoUrl = await this.uploadFile(this.uploadedLogo)
        ...
        for (const [key, value] of Object.entries(this.uploadedFiles)) {
          if (value.files && value.files.length) {
            const urls = await Promise.all(value.files.map((file) => this.uploadFile(file)))
            fileUploads[key] = urls
            submitData[key] = urls // Store URLs in submitData
          }
        }

        // Submit to store
        await store.createMerchant(submitData)
