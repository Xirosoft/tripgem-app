...
const handleRouteChange = async () => {
  ...
  if (isLoggedIn.value) {
    ...
    // console.log('home', route.path.trim().toLowerCase() === '/')
    // if (route.path.trim().toLowerCase() === '/') {
    //   console.log('ddddd')
    //   router.push({ name: 'tripgemlogin' })
    // }
  }
...
onMounted(async () => {
  await nextTick() // Ensure DOM and route are fully updated
  // console.log('home', route.path.trim().toLowerCase() === '/')
  // console.log('route.path after mount:', route.query)

  if (!isLoggedIn.value) {
    console.log('Current pathj', currentPath)

    const currentPath = route.path.trim().toLowerCase()
    if (currentPath === '/' || currentPath === '/register') {
      console.log('Root URL or Register page detected')
      // router.push({ name: 'tripgemlogin' });
    }
  }
  handleRouteChange()
