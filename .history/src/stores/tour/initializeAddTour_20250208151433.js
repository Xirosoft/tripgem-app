...
export function initializeAddTour(handleThumbnailUpload, handleVideoGalleryUpload) {
  'use strict'
  ...
  ;(function () {
    ...
    const initializeDropzone = (elementId, uploadHandler) => {
      ...
      dropzoneInstance.on('addedfile', (file) => {
        if (dropzoneInstance.files.file > 1) {
          dropzoneInstance.removeFile(dropzoneInstance.file[0])
        }
      })
    ...
    window.dropzoneImageGallery = initializeDropzone('#image_gallery', handleVideoGalleryUpload)
