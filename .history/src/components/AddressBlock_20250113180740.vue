...
export default {
  ...
  setup(props, { emit }) {
    ...
    const loadCountries = async () => {
      try {
        ...
        if (response.data) {
          ...
          setTimeout(() => {
            initializeSelect2('country-select')
          }, 100)
        }
      } catch (error) {
    ...
    const loadCities = async () => {
      ...
      try {
        ...
        locationData.value.cities.forEach((city) => {
          $select.append(new Option(city.name, city.name))
        })

        initializeSelect2('city-select')
      } catch (error) {
    ...
    const initializeSelect2 = (selectId) => {
      ...
      $select
        ...
        .on('select2:select', (e) => {
          ...
          if (selectId === 'country-select') loadProvinces()
          if (selectId === 'province-select') loadCities()
    ...
    // Replace initializeLocationFields with this:
