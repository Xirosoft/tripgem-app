...
export default {
  ...
  computed: {
    filteredMenuItems() {
      const result = this.uCan('read', 'merchant_module')
      console.log(result)

      // console.log(this.uCan('read', { module: 'merchant_module', action: 'read', allowed: true }))

      return this.menuItems.filter((item) => this.uCan(item.permission.action, item.permission))
    },
  },
  methods: {
    menuToggle(event) {
    ...
    filteredSubmenuItems(submenu) {
      return submenu.filter(
        (subitem) =>
          !subitem.permission || this.uCan(subitem.permission.action, subitem.permission),
      )
    },
  },
}
