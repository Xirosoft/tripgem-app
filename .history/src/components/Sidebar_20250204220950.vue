...
export default {
  ...
  methods: {
    ...
    uCan(action, module) {
      ...
      const permissions = this.$store.state.permissions // Assuming permissions are stored in Vuex
      const result = permissions.some((perm) => perm.module === module && perm.action === action)
      console.log(`Permission check result for action: ${action}, module: ${module} is ${result}`)
      return result
    },
  },
}
