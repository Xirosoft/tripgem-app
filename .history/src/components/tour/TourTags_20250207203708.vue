...
const initializeSelect2 = () => {
  ...
  setTimeout(() => {
    $(tagSelectRef.value)
      ...
      .val(selectedTags.value) // Set selected values
      .trigger('change')
      .on('select2:select', async (e) => {
        ...
        if (!selectedTags.value.includes(data.id)) {
          selectedTags.value.push(data.id) // Ensure new selection is added
        }
      })
      .on('select2:unselect', (e) => {
        const data = e.params.data
        selectedTags.value = selectedTags.value.filter((id) => id !== data.id) // Remove unselected tags
      })
  }, 100)
}
