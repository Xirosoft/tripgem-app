...
const addTag = async (tag) => {
  try {
    const response = await axios.post(`${config.apiUrl}/tour/tag/add`, {
      name: tag.tag_name, // Use tag_name for name
      slug: tag.tag_slug || tag.tag_name.toLowerCase().replace(/\s+/g, '-'), // Generate slug if not provided
      description: tag.tag_description || '' // Use empty string if description is not provided
    }, {
      headers: config.getHeaders(),
    })
  ...
  nextTick(() => {
    $(tagSelectRef.value)
      ...
      .on('select2:select', async (e) => {
        ...
        selectedTags.value.push(data.id) // Add selected tag to selectedTags
      })
      .on('select2:unselect', (e) => {
        const data = e.params.data
        selectedTags.value = selectedTags.value.filter((id) => id !== data.id) // Remove unselected tag from selectedTags
      })
  })
})
