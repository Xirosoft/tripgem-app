    ...
    $(locationSelectRef.value)
      ...
      .on('select2:select', (e) => {
        ...
        const selectedLocation = locations.value.find(
          (location) => location.location_id.toString() === selectedId,
        )
        currentLocation.value = selectedLocation ? selectedLocation.location_name : null
        emit('location-change', selectedLocation)
      })
      .val(
        locations.value.find((location) => location.location_name === props.selectedLocation)
          ?.location_id || '',
      )
      .trigger('change')
  })
  ...
  (newLocation) => {
    if (newLocation) {
      ...
      nextTick(() => {
        ...
        $(locationSelectRef.value)
          .val(selectedLocation ? selectedLocation.location_id.toString() : '')
          .trigger('change')
      })
    }
  },
