...
const initializeSelect2 = () => {
  ...
  if ($.fn.select2 && $(categorySelectRef.value).data('select2')) {
    $(categorySelectRef.value).select2('destroy') // Destroy previous instance if exists
  }
...
watch(
  ...
  (newCategory) => {
    if (newCategory) {
      ...
      nextTick(() => {
        $('#category')
          .val(newCategory.map((cat) => cat.id))
          .val(
            categories.value.find((categoriy) => categoriy.location_name === newCategory)
              ?.location_id || '',
          )
          .trigger('change')
      })
