...
const locationId = ref(props.selectedLocationId)

watch(
  () => props.selectedLocationId,
  (newVal) => {
    locationId.value = newVal
    $('#location-select').val(newVal).trigger('change')
  },
)
...
onMounted(async () => {
  ...
  nextTick(() => {
    ...
    $('#location-select').val(locationId.value).trigger('change')
  })
})
</script>
