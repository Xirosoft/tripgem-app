...
export default {
  ...
  setup(props, { emit }) {
    const API_URLs = {
      ...
      cities: 'https://countriesnow.space/api/v0.1/countries/state/cities', // Fixed API endpoint
    }
    ...
    const loadCities = async () => {
      ...
      if (!addressData.value.country || !addressData.value.province) return
      ...
      try {
        ...
        console.log('Loading cities for:', {
        ...
        if (response.data?.data) {
          locationData.value.cities = response.data.data
            .filter((city) => city) // Filter out any null/undefined values
            .map((city) => ({
              id: city,
              name: city,
            }))
          ...
          // Add default option
          $select.append(new Option('Select City', '', true, true))
    ...
    const initializeSelect2 = async (selectId) => {
      ...
      $select
        ...
        .on('select2:select', async (e) => {
          addressData.value[selectId.split('-')[0]] = e.target.value
          if (selectId === 'country-select') await loadProvinces()
          if (selectId === 'province-select') await loadCities()
          if (selectId === 'city-select') updateZipCode()
        })
    ...
    onBeforeUnmount(() => {
      ;['country-select', 'province-select', 'city-select'].forEach((selectId) => {
        const $select = $(`#${selectId}`)
        if ($select.hasClass('select2-hidden-accessible')) {
          $select.select2('destroy')
        }
