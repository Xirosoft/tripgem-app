      ...
      if (role) {
        ...
        try {
          ...
          console.log('Role permissions:', rolePermissions)
            ...
            this.permissions.forEach((viewPermission) => {
            const matchedPermission = rolePermissions.find(
              (p) => p.permission_id === viewPermission.permission_id
            );
            if (matchedPermission) {
              const readCheckbox = document.getElementById(`edit_${viewPermission.permission_name}_read`);
              const writeCheckbox = document.getElementById(`edit_${viewPermission.permission_name}_write`);
              const createCheckbox = document.getElementById(`edit_${viewPermission.permission_name}_create`);

              if (readCheckbox) readCheckbox.checked = matchedPermission.can_read === '1';
              if (writeCheckbox) writeCheckbox.checked = matchedPermission.can_write === '1';
              if (createCheckbox) createCheckbox.checked = matchedPermission.can_create === '1';
            }
            });
          })
        } catch (error) {
