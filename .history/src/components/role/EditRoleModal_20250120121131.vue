...
export default defineComponent({
  ...
  methods: {
    ...
    async mapRolePermissions(roleId, permissions) {
      try {
        const userId = useAuthStore().userId // Set userId using useAuthStore
        for (const permission of permissions) {
          ...
          const response = await axios.post(
            `${config.apiUrl}/roles/permissions/mapping/add`,
            payload,
            {
              headers: config.getHeaders(),
            },
    ...
    async editRole() {
      ...
      try {
        ...
        if (!rolesListStore.error) {
          ...
          console.log('permissiosInsert:', this.fetchPermissions())

          const response = await axios.get(
            `${config.apiUrl}/permissions/user/${useAuthStore().userId}`,
            {
              headers: config.getHeaders(),
            },
          )
          TgHandlePusherChannel('permissions-update', 'update', response.data) // Trigger Pusher update
          console.log('response:', response)
