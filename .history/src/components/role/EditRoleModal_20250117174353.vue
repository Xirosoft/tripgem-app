      ...
      if (role) {
        ...
        try {
          ...
          rolePermissions.forEach((permission) => {
            const readCheckbox = document.getElementById(`edit_${permission.permission_id}_read`)
            const writeCheckbox = document.getElementById(`edit_${permission.permission_id}_write`)
            const createCheckbox = document.getElementById(
              `edit_${permission.permission_id}_create`,
            )
            console.log(`edit_${permission.permission_id}_read`)

            if (readCheckbox) readCheckbox.checked = permission.can_read === '1'
            if (writeCheckbox) writeCheckbox.checked = permission.can_write === '1'
            if (createCheckbox) createCheckbox.checked = permission.can_create === '1'
          })
        } catch (error) {
