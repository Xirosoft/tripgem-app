  ...
  methods: {
    ...
    async addRole() {
      ...
      try {
        ...
        if (response && response && response.role_id && !rolesListStore.error) {
          const roleId = response.role_id // Ensure role_id is correctly retrieved
          ...
          this.toast.error(rolesListStore.error || 'Error creating role')
        }
      } catch (error) {
        this.toast.error('Error adding role')
