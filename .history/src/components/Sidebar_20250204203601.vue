  ...
  computed: {
    filteredMenuItems() {
      ...
      const filteredItems = this.menuItems.filter((item) => {
        const canAccess =
          item.permission.allowed && this.uCan(item.permission.action === true, item.permission)
        console.log(
          `Item: ${item.title}, Allowed: ${item.permission.allowed}, Can Access: ${canAccess}`,
        )
        return canAccess
      })
  ...
  methods: {
    ...
    filteredSubmenuItems(submenu) {
      ...
      const filteredSubItems = submenu.filter((subitem) => {
        const canAccess =
          subitem.permission.allowed && this.uCan(subitem.permission.action, subitem.permission)
        console.log(
          `Subitem: ${subitem.title}, Allowed: ${subitem.permission.allowed}, Can Access: ${canAccess}`,
        )
        return canAccess
      })
