...
export default {
  ...
  computed: {
    filteredMenuItems() {
      const result = this.uCan('read', 'user_dashboard')
      console.log('Result:', result)
      // console.log(uCan('read', 'user_dashboard'))

      // console.log('Menu Items:', this.menuItems)
      const filteredItems = this.menuItems.filter((item) => {
        const canAccess = this.uCan(item.permission.action, item.permission)
        // console.log(`Item: ${item.title}, Can Access: ${canAccess}`)
        return canAccess
      })
  ...
  methods: {
    ...
    filteredSubmenuItems(submenu) {
      ...
      const filteredSubItems = submenu.filter((subitem) => {
        const canAccess = this.uCan(subitem.permission.action, subitem.permission)
        console.log(`Subitem: ${subitem.title}, Can Access: ${canAccess}`)
        return canAccess
      })
      console.log('Filtered Submenu Items:', filteredSubItems)
      return filteredSubItems
    },
    uCan(action, permission) {
      // console.log(`Checking permission for action: ${action}, permission:`, permission)
      // Assuming uCan is a method that checks permissions
      // Replace this with the actual implementation of uCan
      const result = true // Replace with actual permission check logic
      // console.log(
      //   `Permission check result for action: ${action}, permission:`,
      //   permission,
      //   'is',
      //   result,
      // )
